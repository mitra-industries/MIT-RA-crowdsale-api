...
import (
    "mit-ra-crowdsale-api/solidity/bindings/mit-ra_crowdsale"
    ...
    modelsCommon "mit-ra-crowdsale-api/common/models"
...
type Crowdsale struct {
    *ethereum.Contract
    Crowdsale *mit-ra_crowdsale.Mit-raCrowdsale
}

func Init() error {
    c := ethereum.NewContract(viper.GetString("ethereum.address.crowdsale"))
    c.InitEvents(mit-ra_crowdsale.Mit-raCrowdsaleABI)

    s, err := mit-ra_crowdsale.NewMit-raCrowdsale(c.Address, c.Wallet.Connection)
    if err != nil {
...
func (s *Crowdsale) Deploy(params *models.CrowdsaleDeployParams) (*common.Address, *types.Transaction, error) {
    ...
    address, tx, _, err := mit-ra_crowdsale.DeployMit-raCrowdsale(
        &bind.TransactOpts{
